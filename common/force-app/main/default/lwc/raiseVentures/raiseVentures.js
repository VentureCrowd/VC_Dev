...
export default class RaiseVentures extends NavigationMixin(LightningElement) {
    @track raiseDetails = {};
    @track productId;
    @api isLoading = false;
    @track error;
    ...
    navLinksRetail = [
        { name: 'founders', label: 'Meet the Founders' },
     ...
     */
    get isLive(){
        return ((this.raiseStatus === 'Live') && (this.isPreview !== true)) || (this.isPreview === true);
    }
    ...
    handleInvestClick() {
        ...
        if (amountInput) {
            ...
            let minInvestAmount = parseFloat(this.raiseDetails.minInvestAmount.replace(/[`,`]/g, ''));
            if (isNaN(amount) || amount < minInvestAmount) {

                // CORE TOAST MODIFICATION - KENDRICK
                this.errorBodyFields = [
                    {label: `Investment amount value must be at least $${ this.raiseDetails.minInvestAmount}`}
                ]

                this.errorTitle = `There are errors in the following field${this.errorBodyFields.length > 1 ? 's' : ''} (${this.errorBodyFields.length}):`;
                ...
                //     new ShowToastEvent({
                //         title: 'Invalid Input',
                //         message: 'Value must be at least $' + this.raiseDetails.minInvestAmount,
                //         variant: 'error'
                //     })
                // );
                return;
    ...
    async getRaiseDetails(raiseId){
        try {
            ...
            .filter(data => data.key === raiseData.investmentProductType)
    ...
    connectedCallback(){
        ...
        this.getQueryParameters().then((result) => {
            ...
            const foundersPromise = this.getTeamMembersList(this.raiseId, 'Founder')
                .then((result) => {
                    this.founders = [...result];
                }).catch((err) => {
                    console.error(err);
                });
