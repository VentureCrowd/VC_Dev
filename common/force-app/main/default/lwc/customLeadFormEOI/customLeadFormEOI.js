...
import createEoiLead from '@salesforce/apex/MyCompaniesController.createEoiLead';
...
export default class CustomLeadFormEOI extends LightningElement {

    @api isCaptchValidated = false;
    @api EOI_ID;
    @api idPreview;
    navigateTo;
    @track datamap = {};
    @track showSpinner = false;
    @track investmentAmount = false;
    @track utm_content;
    @track utm_medium;
    @track utm_campaign;
    @track utm_source;
    ...
    connectedCallback() {
        //* Assign idPreview to EOI_ID if EOI_ID is undefined or falsy
        this.EOI_ID = this.EOI_ID || this.idPreview;
        this.datamap['Are_you_located_in_Australia__c'] = true;
        this.datamap['Wholesale_Status__c'] = "No";
        this.datamap['Related_EOI__c'] = this.EOI_ID;
        this.datamap['Status'] = 'New';
        this.datamap['Investment_Amount__c'] = 'New';
        this.datamap['OwnerId'] = '0052v00000glEDk';
        this.datamap['Email_Opt_In__c'] = true;
        this.getUtmParamaters();
    }
    ...
    //? Method to handle reCAPTCHA token verification
    //! Comment console.log
    captchaLoaded(evt) {}
        ...
        if (data && data.includes('****CaptchToken****')) {
            ...
            verifyrecaptchaToken({ token })
                .then((r) => {
                    ...
                    console.log('This is the output of the captcha' + r)
                })
                .catch((e) => {
                    console.log(e);
                });
    ...
    getUtmParamaters() { 
        if (this.pageRef) {
            ...
            this.utm_keyword = urlParams.get('utm_keyword');

            this.datamap['utm_content__c'] = this.utm_content;
            this.datamap['utm_medium__c'] = this.utm_medium;
            this.datamap['utm_campaign__c'] = this.utm_campaign;
            this.datamap['utm_source__c'] = this.utm_source;
            this.datamap['utm_keyword__c'] = this.utm_keyword;
        }
    }

    handleChange(event) {  
        if(event.target.name == 'FirstName') { 
            this.datamap['FirstName'] = event.target.value;
        }
        else if(event.target.name == 'LastName') { 
            this.datamap['LastName'] = event.target.value;
        }
        else if(event.target.name == 'Email') { 
            this.datamap['Email'] = event.target.value;
        }
        else if(event.target.name == 'countryLabel') { 
            this.countyCode = event.detail.value;
        }
        else if(event.target.name == 'MobilePhone') { 
            let phoneNumber = event.target.value
            this.datamap['MobilePhone'] = '+' + this.countyCode + phoneNumber;
        }
        else if(event.target.name == 'Investment_Amount__c') { 
            if(event.target.value != "" && event.target.value) { 
                this.investmentAmount = true;
            }
            this.datamap['Investment_Amount__c'] = event.target.value;
        }
        else if(event.target.name == 'Are_you_located_in_Australia__c') { 
            if(event.target.value == 'Yes') { 
                this.datamap['Are_you_located_in_Australia__c'] = true;
            } else { 
                this.datamap['Are_you_located_in_Australia__c'] = false;
            }
        }
        else if(event.target.name == 'Wholesale_Status__c') { 
            if(event.target.value == 'Wholesale') { 
                this.datamap['Wholesale_Status__c'] = 'Yes';
            } else { 
                this.datamap['Wholesale_Status__c'] = 'No';
            }
        }
    ...
    createleadRecord() {
        if (isCaptchValidated) {
            ...
            let webSource = this.extractIdentifierFromUrl(currentPageUrl);
            this.datamap['Web_Source__c'] = webSource;
            let datamap = this.datamap;
            if(isformvalid && this.investmentAmount && this.datamap['Are_you_located_in_Australia__c'] == true) { 
                createEoiLead({datamap})
                .then(r =>{
                    // If company creation is successful
                    if(r == 'Success'){  
                    this.showSpinner = false;
                    // Show success toast message
                    this.showtoast('Form submission successful, redirecting…','success');
                    const url = 'https://www.venturecrowd.com.au/s/typ-eoi';
                    window.location.href = url;
                    } else{
                        this.showSpinner = false;
                        this.showtoast('Error in network. Please reload the page and try again.',r,'error');
                    }                
                })
                .catch(e => {
                    this.showSpinner = false;
                    this.showtoast('Error in network. Please reload the page and try again.', e,'error');
                })
            } else { 
                this.showSpinner = false;
                if(this.datamap['Are_you_located_in_Australia__c'] == true) { 
                    this.showtoast('Please enter all fields in the form to submit your EOI.','error');
                } else { 
                    this.showtoast('We are only accepting Australian based EOI’s at the moment.','error');
                }
            }
        } else {
            this.showSpinner = false;
            this.showtoast('Captcha Failed.','error');
        }
    }

    extractIdentifierFromUrl(url) {
        // Split the URL by "/" to separate the path
        const pathParts = url.split('/');
    
        // Get the last part of the path
        let identifier = pathParts[pathParts.length - 1];
    
        // Check for fragment identifier (hash)
        const fragmentIndex = identifier.indexOf('#');
        if (fragmentIndex !== -1) {
            // If a fragment identifier exists, remove it
            identifier = identifier.substring(0, fragmentIndex);
        } else {
            // If no fragment identifier, check for query parameters
    ...
    get phoneoptions() {
        let phoneNumberoptns = [
            ...
            { value: '1767', label: 'Dominican Republic +1767' },
            ...
            { value: '691', label: 'Federated States of Micronesia +691' },
            ...
            { value: '962', label: 'Hashemite Kingdom of Jordan +962' },
            ...
            { value: '95', label: 'Myanmar [Burma] +95' },
            ...
            { value: '872', label: 'Pitcairn Islands +872' },
            { value: '48', label: 'Poland +48' },
            { value: '351', label: 'Portugal +351' },
            { value: '1939', label: 'Puerto Rico +1939' },
            { value: '974', label: 'Qatar +974' },
            { value: '82', label: 'Republic of Korea +82' },
            { value: '370', label: 'Republic of Lithuania +370' },
            ...
            { value: '590', label: 'Sint Maarten +590' },
            ...
            { value: '230', label: 'U.S. Minor Outlying Islands' },
